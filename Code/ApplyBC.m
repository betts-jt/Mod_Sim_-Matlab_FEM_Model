function [Global_Mat, Global_Vec] = ApplyBC(BC1T,BC1V,BC2T,BC2V, Data,...
    Global_Mat, Global_Vec)
...
if BC1T == 'D' % Check if the first boundary is a Dirichlet boundary
    ...
    % Set the top value of the global source vector to the input value of BC1
    Global_Vec(1) = BC1V; 
elseif BC1T == 'N' % Check if the first boundary is a Neumann boundary
    % add the value of -D(dc/dx)x=0 to the first element of the source term vector
    Global_Vec(1) = Global_Vec(1) + -BC1V; 
else % If neither 1 or 2 are entered for BC1
    error(['Incorrect boundary condition type entered for BC1. Enter either'...
        ' D for a Dirichlet boundary condition or N for a Neumann boundary condition'])
end
...
elseif BC2T == 'N' % Check if the second boundary is a Neumann boundary
    % add the value of D(dc/dx)x=1 to the last element of the source term vector
    Global_Vec(end) = BC2V; 
else % If neither 1 or 2 are entered for BC1
    error(['Incorrect boundary condition type entered for BC2. Enter either'...
        'D for a Dirichlet boundary condition or N for a Neumann boundary condition'])
end
